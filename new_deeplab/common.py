...
class ModelOptions(
    ...
    def __new__(cls,
        ...
        if params.dense_prediction_cell_json:
            with tf.gfile.Open(params.dense_prediction_cell_json, 'r') as f:
                dense_prediction_cell_config = json.load(f)
        decoder_output_stride = None
        if params.decoder_output_stride:
            decoder_output_stride = [
                int(x) for x in params.decoder_output_stride]
            if sorted(decoder_output_stride, reverse=True) != decoder_output_stride:
                raise ValueError('Decoder output stride need to be sorted in '
                                 'descending order.')
        image_pooling_crop_size = None
        if params.image_pooling_crop_size:
            image_pooling_crop_size = [int(x) for x in params.image_pooling_crop_size]
        image_pooling_stride = [1, 1]
        if params.image_pooling_stride:
            image_pooling_stride = [int(x) for x in params.image_pooling_stride]
        return super(ModelOptions, cls).__new__(
            cls, params, outputs_to_num_classes, crop_size, atrous_rates, output_stride,
            preprocessed_images_dtype, params.merge_method,
            params.add_image_level_feature,
            image_pooling_crop_size,
            image_pooling_stride,
            params.aspp_with_batch_norm,
            params.aspp_with_separable_conv, params.multi_grid, decoder_output_stride,
            params.decoder_use_separable_conv, params.logits_kernel_size,
            params.model_variant, params.depth_multiplier, params.divisible_by,
            params.prediction_with_upsampled_logits, dense_prediction_cell_config,
            params.nas_stem_output_num_conv_filters, params.use_bounded_activation)
